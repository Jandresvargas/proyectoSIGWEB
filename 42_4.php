...
<body>
    ...
    <button id="cancelBufferButton">Cancelar buffer</button>
    <table id="data-table">
        <thead>
            <tr>
                <th>ID</th>
    ...
    <script>
        ...
        var buffer; // Variable para almacenar el buffer
        ...
        $.post("php/cargar_motorbike2.php",
            {
                ...
                if(status == 'success') {
                    motos = eval('('+data+')');
                    var motosLayer = L.geoJSON(motos, {
                        onEachFeature: info_popup
                    });
                    
                    motosLayer.eachLayer(function (layer) {
                        layer.setZIndexOffset(1000);
                    });
                    leyenda.addOverlay(motosLayer, 'Talleres de motos');
                }
            });

        // Obtener la ubicación actual del usuario y datos de la base de datos al presionar el botón "Obtener datos"
        document.getElementById('getDataButton').addEventListener('click', function() {
            navigator.geolocation.getCurrentPosition(function(position) {
                ...
                var bufferRadius = parseInt(document.getElementById('bufferSlider').value); // Valor del radio del buffer en metros
        ...
        // Evento de escucha para cancelar el buffer al presionar el botón "Cancelar buffer"
        document.getElementById('cancelBufferButton').addEventListener('click', function() {
            if (buffer) {
                buffer.remove(); // Eliminar el buffer del mapa
            }
        });
